...
int main(int argc,char **argv) {
   ...
   */
/**** your code here ****/
  MPI_Exscan(&my_number_of_elements,&my_first_index,
  ...
  MPI_Gather(&my_number_of_elements, 1, MPI_INT, size_buffer, 1, MPI_INT, 0, comm);
...
// where are they going to go in the big buffer?
  ...
  if (procno==0){
    ...
    for(int i = 1; i < nprocs; i++){
      displ_buffer[i] = displ_buffer[i - 1] + size_buffer[i - 1];
    }
...
//      );
   ...
   */
  MPI_Gatherv(my_elements, my_number_of_elements, MPI_INT, gather_buffer, size_buffer, displ_buffer, MPI_INT, 0, comm);
